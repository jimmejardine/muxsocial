...
async fn compose_greeting_round_trips_through_wasm() {
    ...
    let greeting_message: String = muxsocial_client_wasm_instance.compose_greeting("wasm-test".to_string()).await;
    assert_eq!(greeting_message, "Hello, wasm-test! Greetings again from muxsocial-lib.");
}
